                             ...
                             effect_handle_t*    pHandle)
{
    ...
    if (!st) { delete ctx; return -ENOMEM; }
                         ...
                         audio_buffer_t* outBuf)
{
    ...
    if (!ctx || !inBuf || !outBuf) return -EINVAL;
                         ...
                         void*    pReplyData)
{
    ...
    case EFFECT_CMD_INIT:
        ...
        if (!replySize || *replySize < sizeof(int) || !pReplyData) return -EINVAL;
        *(int*)pReplyData = 0;
        return 0;
    ...
    case audioshift::CMD_SET_PITCH_RATIO: {
        if (cmdSize < sizeof(float) || !pCmdData) return -EINVAL;
        const float ratio = *(const float*)pCmdData;
        if (ratio <= 0.0f || ratio > 2.0f) return -EINVAL;
    ...
    case audioshift::CMD_GET_LATENCY_MS:
        if (!pReplyData || !replySize || *replySize < sizeof(float)) return -EINVAL;
...
static int effectGetDescriptor(effect_handle_t     self,
                                effect_descriptor_t* pDescriptor) {
    auto* ctx = reinterpret_cast<audioshift::AudioShiftContext*>(self);
    if (!ctx || !pDescriptor) return -EINVAL;
    *pDescriptor = audioshift::AUDIOSHIFT_EFFECT_DESCRIPTOR;
    return 0;
}
